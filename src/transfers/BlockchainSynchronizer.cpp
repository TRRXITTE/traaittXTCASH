...
namespace CryptoNote
{
    ...
    bool BlockchainSynchronizer::removeConsumer(IBlockchainConsumer *consumer)
    {
        ...
        if (result)
        {
            m_logger(INFO, BRIGHT_WHITE) << "Consumer removed, consumer " << consumer << ", count "
                                         << m_consumers.size();
        }
        else
        {
    ...
    void BlockchainSynchronizer::load(std::istream &in)
    {
        ...
        if (genesisBlockHash != m_genesisBlockHash)
        {
            ...
            m_logger(ERROR, BRIGHT_RED) << message << ", read " << genesisBlockHash << ", expected "
                                        << m_genesisBlockHash;
            throw std::runtime_error(message);
        }
        ...
        std::unordered_set<Crypto::Hash> &poolIntersection) const
    {
        ...
        m_logger(DEBUGGING) << "Pool union size " << poolUnion.size() << ", intersection size "
                            << poolIntersection.size();
    }
    ...
    BlockchainSynchronizer::GetBlocksRequest BlockchainSynchronizer::getCommonHistory()
    {
        ...
        m_logger(DEBUGGING) << "Common history: start block index " << request.syncStart.height
        ...
        const std::vector<CompleteBlock> &blocks)
    {
        ...
        for (auto &kv : m_consumers)
        {
            ...
            {
                m_logger(DEBUGGING) << "Detach consumer, consumer " << kv.first << ", block index "
                                    << result.detachHeight;
            ...
            if (result.hasNewBlocks)
            {
                ...
                if (addedCount > 0)
                {
                    ...
                    {
                        m_logger(ERROR, BRIGHT_RED) << "Failed to add " << (blockCount - addedCount) << " blocks of "
                                                    << blockCount << " to consumer, consumer " << kv.first;
                        hasErrors = true;
                    }
        ...
        if (lastBlockIndex != std::numeric_limits<uint32_t>::max())
        {
            ...
            m_logger(DEBUGGING) << "Last block hash " << lastBlockId << ", index "
                                << (interval.startHeight + lastBlockIndex);
        }
    ...
    void BlockchainSynchronizer::removeOutdatedTransactions()
    {
        ...
        if (!ec)
        {
            ...
            for (auto &consumer : m_consumers)
            {
                ...
                if (ec)
                {
                    m_logger(ERROR, BRIGHT_RED) << "Failed to process outdated pool transactions: " << ec << ", "
                                                << ec.message() << ", consumer " << consumer.first;
                    break;
                }
            }
        }
                ...
                else
                {
                    ...
                    { // get intersection ok
                        ...
                        else
                        {
                            ...
                            intersectionResponse.deletedTxIds.assign(
    ...
    std::error_code BlockchainSynchronizer::processPoolTxs(GetPoolResponse &response)
    {
        m_logger(DEBUGGING) << "Starting to process pool transactions, added " << response.newTxs.size() << ':'
                            << TransactionReaderListFormatter(response.newTxs) << ", deleted "
                            << response.deletedTxIds.size() << ':'
        ...
        std::error_code error;
        {
            ...
            for (auto &consumer : m_consumers)
            {
                ...
                if (error)
                {
                    m_logger(ERROR, BRIGHT_RED) << "Failed to process pool transactions: " << error << ", "
                                                << error.message() << ", consumer " << consumer.first;
                    break;
                }
            }
        }
