...
{
  public:
    ThreadSafePriorityQueue(): m_shouldStop(false) {}

    ThreadSafePriorityQueue(bool startStopped): m_shouldStop(startStopped) {}
    ...
    template<typename Iterator> bool push_n(Iterator begin, Iterator end)
    {
        /* Acquire the lock */
    ...
    /* Delete the front item from the queue, without acquiring the lock */
    void pop_unsafe()
    {
        m_priorityQueue.pop();
    }

    /* Get the top item, without acquiring the lock */
    T top_unsafe()
    {
